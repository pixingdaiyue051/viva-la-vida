package com.teuqueno.service.base.impl;
...
public class BaseServiceWrapperImpl<D extends Mapper<T>, T> implements BaseService<T> {
	@Autowired
	protected D dao;
	...
	public int insertSelective(T entity) throws Exception {
		...
		if (result > 0) {
			...
			SolrInputDocument doc = ToolsUtil.castToSolrInputDoc(entity);
			solrUtil.saveSingle(doc);
		}
		return result;
	}
